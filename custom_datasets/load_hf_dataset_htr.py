...
class ours_VAECollate:
    ...
    def __call__(self, batch):
        # images (already mapped to 'rgb.png' / 'bw.png' in ours_DataLoaderManager)
        rgb_imgs = [sample['rgb.png'] for sample in batch]
        bw_imgs  = [sample['bw.png']  for sample in batch]
        rgb_imgs_padded = DataProcessor.pad_images_fixed(rgb_imgs)
        bw_imgs_padded  = DataProcessor.pad_images_fixed(bw_imgs)

        # real writer_id from the JSON
        writer_ids = [sample['json']['writer_id'] for sample in batch]

        # IMPORTANT: use 'gen_text' for lengths
        gen_texts = [sample['json']['gen_text'] for sample in batch]
        texts_len = torch.LongTensor([len(t) for t in gen_texts])

        # encoded_text was already computed in the dataset .map(...)
        encoded_texts = [
            torch.LongTensor(sample['encoded_text']) for sample in batch
        ]
...
class ours_DataLoaderManager:
    ...
    def create_dataset(self, split: str, model_type: str):
        ...
        dataset = (
            ...
            .decode("pil")
            .map(lambda sample: {
                "rgb.png": transform(sample["gen.rgb.png"].convert("RGB")),
                "bw.png":  transform(sample["gen.bw.png"].convert("L")),
                "json": sample["json"],
